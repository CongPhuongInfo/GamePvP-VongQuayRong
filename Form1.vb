...
Public Class Form1
    ...
    Private animalCenters(VongQuayRongGame.TOTAL_SLOTS - 1) As PointF
    Private boardRect As Rectangle

    ' ------------------- Jackpot (o "No Hu") -------------------
    Private currentJackpotPool As Long = VongQuayRongGame.JACKPOT_SEED
    Private pnlJackpotBanner As Panel    ' banner rieng phia tren vong quay, khong con de len con vat nua
    ...
    ' ------------------- Nhat ky ket qua (con vat da ra gan day) -------------------
    ...
    Private Sub Hub_ClientConnected(seatIndex As Integer)
        ...
        If spinHistory.Count > 0 Then
            hub.SendToClient(seatIndex, "VQR_HISTORY:" & String.Join(",", spinHistory))
        End If
    ...
    Private Sub BuildSpinSequence(targetIndex As Integer)
        ...
        Dim n As Integer = VongQuayRongGame.TOTAL_SLOTS ' 13 o (12 con vat + No Hu)
    ...
    Private Sub ShowGamePanel()
        ...
        ' Banner Quy Jackpot rieng, nam TREN vong quay (khong con de de len hang con vat dau tien nua)
    ...
    ''' Dat rieng ben tren vong quay de khong con de len hang con vat dau tien nhu truoc.</summary>
    ...
    ' ============================================================
    '  NHAT KY KET QUA (con vat / No Hu da ra trong nhung van gan day)
    ' ============================================================
    ...
    ''' <summary>Ve tung o nhat ky: icon tron nho (sprite con vat/No Hu tu Assets, fallback mau + ten neu thieu anh)
    ...
    Private Sub HistoryPanel_Paint(sender As Object, e As PaintEventArgs)
        ...
        For i = 0 To spinHistory.Count - 1
            ...
            If idx = VongQuayRongGame.JACKPOT_INDEX Then
                If jackpotSlotImage IsNot Nothing Then
            ...
            Else
                ...
                If animalImages(idx) IsNot Nothing Then
                    DrawSpriteCircular(g, animalImages(idx), c, r, Color.FromArgb(90, 90, 90), 1)
                Else
    ...
    Private Sub RecomputeAnimalCenters()
        ...
        Dim n As Integer = VongQuayRongGame.TOTAL_SLOTS ' 12 con vat + 1 o No Hu, chia deu quanh vien
        Dim i As Integer
        For i = 0 To n - 1
    ...
    Private Sub BoardPanel_Paint(sender As Object, e As PaintEventArgs)
        ...
        ' O "No Hu" (Jackpot) - o thu 13, ngoai 12 con vat
        Dim jIdx As Integer = VongQuayRongGame.JACKPOT_INDEX
        Dim jc As PointF = animalCenters(jIdx)
        ...
        ElseIf jIdx = selectedAnimalIndex Then
        ...
        For jSeatIdx = 0 To 3
            If lockedAnimalBySeat(jSeatIdx) = jIdx Then
    ...
    Private Sub BoardPanel_MouseClick(sender As Object, e As MouseEventArgs)
        ...
        Dim i As Integer
        For i = 0 To VongQuayRongGame.TOTAL_SLOTS - 1
            Dim c As PointF = animalCenters(i)
